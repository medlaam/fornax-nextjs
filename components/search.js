...
const Search = () => {
  ...
  return (
    <div className={`${styles.searchContainer} container`}>
      <div className=" overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-0 sm:top-20 z-50 justify-center items-center md:inset-0 h-modal sm:h-full" id="large-modal">
        <div className={`relative px-4 w-full md:w-2/3 max-w-4xl h-full md:h-auto ${styles.searchModal}`}>
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div className="flex justify-between items-center rounded-t border-b dark:border-gray-600">
              <input className="rounded focus:outline-none	w-full" type="text" placeholder="Search here" onChange={(e) => {setSearchTerm(e.target.value)}} />

              <button onClick={() => setShowSearch(!showSearch)} type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-3 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="large-modal">
                    ...
                    searchAllBlogs.map((b, i) => {
                      return (
                        <div key={b.id} className="p-4 block border-b-2  overflow-hidden">
                            ...
                            <div onClick={() => setShowSearch(!showSearch)} className="text-3xl">
